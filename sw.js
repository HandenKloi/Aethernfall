const CACHE_NAME='aethernfall-v32';
const CORE=['./','./index.html','./style.css?v=3200','./game.js?v=3200','./manifest.json?v=3200'];
...
self.addEventListener('fetch',e=>{
 ...
 if(app){
  e.respondWith(fetch(r,{cache:'no-store'}).then(res=>{
   if(!res.ok)throw new Error('HTTP '+res.status);const copy=res.clone();caches.open(CACHE_NAME).then(c=>c.put(r,copy));return res
