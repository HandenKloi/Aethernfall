...
function nearbyInteraction(){
 ...
 if(dist(player,z.portal)<135)return {type:'portal'};
 if(dist(player,z.scout)<120)return {type:'scout'};
 let bestLoot=null,bd=105;
 for(const l of lootDrops){const d=dist(player,l);if(d<bd){bd=d;bestLoot=l}}
 if(bestLoot)return {type:'loot',entity:bestLoot};
 const res=nearest(e=>e.kind==='resource',105);
 return res?{type:'resource',entity:res}:null;
}
...
function interact(){
 ...
 if(hit.type==='scout'){
  const s=questState();
  if(s.step===0){openNpcDialog();advanceQuest('scout');}
...
function updatePerformanceMonitor(nowTime,renderMs,frameTimeMs){
 ...
 const t=nowTime;
 if(!perfWindowStart||t-perfWindowStart>=750){
   const elapsed=Math.max(1,t-(perfWindowStart||t));
 ...
 if(ui.perfPill){
   ui.perfPill.classList.toggle('hidden',!settings.monitor);
   if(settings.monitor){
     ui.perfPillFps.textContent=perfActualFps.toFixed(0);
     ui.perfPillMs.textContent=perfAvgFrameMs.toFixed(1);
   }
 }
 if(ui.perf&&settings.monitor){
   ui.perf.classList.remove('hidden');
   ui.perfFps.textContent=perfActualFps.toFixed(0);
